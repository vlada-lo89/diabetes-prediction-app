...
def main():
    ...
    st.write(
        "Это интерактивное веб-приложение, использующее модель машинного "
        "обучения (Random Forest) для классификации наличия диабета на "
        "основе медицинских показателей."
    )
    ...
    pregnancies = st.sidebar.number_input(
        "Число беременностей (Pregnancies)",
        min_value=0, max_value=20, value=1, step=1
    )
    glucose = st.sidebar.slider(
        "Концентрация глюкозы (Glucose)",
        0, 200, 100, 1
    )
    ...
    if st.button("Сделать предсказание"):
        ...
        if prediction[0] == 1:
            st.error(
                f"Вероятно, у пациента ДИАБЕТ. "
                f"(уверенность: {prediction_proba[0][1]:.2f})"
            )
        else:
            st.success(
                f"Вероятно, у пациента НЕТ диабета. "
                f"(уверенность: {prediction_proba[0][0]:.2f})"
            )
